...
"""Tests for `cellarity_cellpainting` package."""
...
def test_submit_job_batch():
    ...
    command = ["bash", "-c", " ".join(["nextflow --help;", "sleep 60"])]
...
def test_s3_access():
    ...
    command = ["bash", "-c", " ".join(["cd /tmp/tmpfs; ", f"aws s3 cp s3://{DATA_S3}/data/iris.csv ./ && head iris.csv"])]
    job_name = "s3-access"
    job_id = "job-{0}-{1}".format(job_name, int(time.time() * 1000))
    submit_data = {
        "jobName": job_id,
